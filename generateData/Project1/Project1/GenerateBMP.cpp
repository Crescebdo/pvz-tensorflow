...
std::string getPath(std::string basic_path, int count) {
	std::string path = basic_path;
	if (!IsPathExist(path)) std::filesystem::create_directory(path);
	path += "\\" + to_string(count) + ".bmp";
	cout << path << endl;
	return path;
}

void savePuzzleToFile(std::string basic_path, int** code, int count) {
	ofstream ofs;
	ofs.open(basic_path + "\\" + to_string(count) + ".txt", ios::trunc);
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < 5; j++) {
			ofs << code[i][j];
			if (j != 4)  ofs << ",";
...
void Screenbmp(int** code, HWND hwnd, string path, int left, int top, int small_width, int small_height, int colN = 5, int rowN = 5) {
	...
	string new_path = getPath(path, counter);
	saveToFile(bmp, new_path.c_str(), hMemoryDC, hBitmap, height);
	DeleteObject(hBitmap);
	savePuzzleToFile(path, code, counter);
	counter++;
