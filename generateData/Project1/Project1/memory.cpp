...
namespace IZE {
	...
	// 返回某植物的名字
	std::string Memory::getName(int plant_type) {
		int index = -1;
	...
	// 检查植物是否有缺少/多余
	std::string Memory::checkEach(int** plants, int* counter, int* goal, bool isDX, bool isBType) {
		missing.clear();
		exessive.clear();
		for (int i = 0; i < TOTAL_NUM; i++) {
	...
	// 检查主题
	std::string Memory::checkTheme(int** plants, int* counter, int theme) {
		if (theme == 0) {
			// 综合
	...
	// 返回某植物的代码
	std::string Memory::getCode(int plant_type) {
		int index = -1;
		for (int i = 0; i < TOTAL_NUM; i++) {
			if (type[i] == plant_type) {
				index = i;
	...
	// 返回某代码对应的植物
	int Memory::getPlantTypeByCode(std::string plant_code) {
		for (int i = 0; i < TOTAL_NUM; i++) {
	...
	// 将阵型转换为代码
	std::string Memory::toCode(int** plants) {
		std::string result = "";
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
	...
	// 读入一段代码并转换
	int** Memory::compilePlantCode(std::string input) {
		...
		for (size_t idx = 0; idx < lines.size(); idx++) {
			...
			std::string line = lines.at(idx);
			int repeat = 1;
			int match = line.find("*", 0);
			if (match != std::string::npos) {
				std::string nextChar = line.substr(match + 1, 1);
				if (!nextChar.empty() && nextChar.find_first_not_of("12345") == std::string::npos) {
					int temp = std::stoi(nextChar);
					if (row + temp <= 5) {
						repeat = temp;
					}
				}
			...
			for (size_t col = 0; col < 5 && col < line.size(); col++) {
				std::string currentChar = line.substr(col, 1);
				if (currentChar == "*") {
					break;
				}
				else {
	...
	// 珍珑检查器部分
	std::string Memory::checkPlants(int theme, bool changeSun, bool changeLevel, bool resetPuffshroom) {
		if (!FindGame())
			if (!FindGame2()) {
				return "找不到游戏";
			}
		...
		std::string result = checkTheme(puzzle, counter, theme);
	...
	// 快捷布阵：读取当前阵型
	std::string Memory::readPlantsToCode() {
		if (!FindGame())
			if (!FindGame2()) {
				return "读取失败：找不到游戏";
			}
		...
		std::string result = toCode(puzzle);
		for (int i = 0; i < 5; i++) {
			delete[] puzzle[i];
		}
		delete[] puzzle;
	...
	// 快捷布阵
	std::string Memory::embattleFromCode(std::string plant_code, bool checkCompliance) {
		if (!FindGame())
			if (!FindGame2()) {
				return "布阵失败：找不到游戏";
			}
	...
	std::string Memory::embattleFromPlants(int** puzzle) {
		if (!FindGame())
			if (!FindGame2()) {
				return "布阵失败：找不到游戏";
			}
